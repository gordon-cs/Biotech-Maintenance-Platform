...
export default function SignUp() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const session = (signUpData as any)?.session ?? sessionData?.session

      if (session && session.user?.id) {
        // authenticated — safe to insert / upsert profile as this user
        const userId = session.user.id

        // Insert into profiles table using the auth user id as the primary key
        ...
        // Depending on role, insert into labs or technicians table
        if (role === "lab") {
          const labInsert = {
            manager_id: userId,
            name: labName || null,
        ...
        // no session — ask user to confirm email and sign in, then complete profile
